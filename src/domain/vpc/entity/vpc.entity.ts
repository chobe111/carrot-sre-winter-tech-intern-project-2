...
export class VpcEntity {
  ...
  static create(dto: VpcDTO) {
    ...
    vpc.cidrBlockAssociationSet = dto.CidrBlockAssociationSet.map((cbas) => VpcCidrBlockAssociationEntity.create(cbas));

    vpc.dhcpOptionsId = dto.DhcpOptionsId;
    vpc.instanceTenancy = dto.InstanceTenancy as InstanceTenancy;

    vpc.ipv6CidrBlockAssociationSet = dto.Ipv6CidrBlockAssociationSet.map((icbas) => VpcIpv6CidrBlockAssociationEntity.create(icbas));
